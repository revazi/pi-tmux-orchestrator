...
9. Configured specialists use conservative deterministic gates. Pass `forceSpecialists` or `--force-specialist ROLE` only when the user explicitly requires that enabled role to run regardless of a skip predicate; never add a classifier model call. Exact-project `customRoles` come from validated user-global configuration; pass `projectCustomRoles=false` only to omit them. Never invent custom role IDs, tools, or contracts.
