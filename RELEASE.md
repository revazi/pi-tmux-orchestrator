...
- [ ] Confirm ordinary new manifests are v5/`broker-v1`, explicit custom-role runs use v7/`broker-v1`, retained older manifests remain readable, no legacy coordination option exists for new runs, workflow payloads stay out of SQLite/status, TUI/RPC share the bridge, agents do not poll or sleep, and tmux hosts/monitors rather than transports workflow messages.
