...
- [ ] Confirm package acceptance uses isolated `pi install <local-package-root>` on the npm-installed tarball root, launches RPC without `--extension`, and discovers exactly ten extension commands plus `skill:tmux-agent-orchestrator` with package provenance.
