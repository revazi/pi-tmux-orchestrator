...
The `0.4.0` source package declares no dependencies or peers, owns no runtime tree, does not bundle Pi, and limits its deterministic 27-file modular artifact to the manifest, canonical MIT license, runtime resources, and operator documentation. Tests, CI, state, sessions, credentials, caches, generated sessions, and development-only files remain excluded. Its scoped manifest requests public access, but publication remains a manual operator action; package metadata, a tarball, and dry-runs do not prove registry or gallery availability. The owner-authorized license is MIT; see [LICENSE.md](LICENSE.md).
