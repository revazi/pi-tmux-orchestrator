...
The npm package declares no dependencies or peers, owns no runtime tree, does not bundle Pi, and limits its deterministic 29-file artifact to the manifest, canonical MIT license, runtime resources, and operator documentation. Tests, CI, state, sessions, credentials, caches, generated sessions, and development-only files remain excluded. See the [MIT License](LICENSE.md).
