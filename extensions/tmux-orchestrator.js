...
function customRoleRegistryConfirmation(data) {
  const registry = data?.custom_role_registry;
  if (!registry?.configured) return "not configured; built-in roles unchanged";
  return [
    `${registry.count} configured; registry-only, not launchable`,
    ...registry.roles.map((role) => (
      `${role.id}: prompt_sha256=${role.prompt_sha256}; skill_sha256=${role.skill_sha256.join(",") || "none"}`
    )),
  ].join("\n");
}
...
function startConfirmation(preview) {
  ...
  return [
    ...
    `Custom specialist registry:\n${customRoleRegistryConfirmation(preview.data)}`,
