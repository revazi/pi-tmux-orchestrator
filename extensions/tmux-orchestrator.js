...
const parameters = {
  ...
  properties: {
    ...
    decisionModel: {
      ...
      description: "Exact available Pi provider/model override for dynamic planning when TypeSafe authentication is absent. Direct TypeSafe Jev configured through /login typesafe or TYPESAFE_API_KEY takes precedence; otherwise this override wins over the strict user-global Pi preferred identity/ordered fallbacks. Planning thinking is capped at medium",
      properties: {
...
async function applyDynamicPlan(pi, ctx, input, project, signal) {
  ...
  const typesafeApiKey = await resolvedTypeSafeApiKey(
    ctx,
