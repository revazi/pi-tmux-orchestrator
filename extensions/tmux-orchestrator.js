...
const parameters = {
  ...
  properties: {
    ...
    decisionModel: {
      ...
      description: "Exact available Pi provider/model override for dynamic planning when TYPESAFE_API_KEY is absent. Direct TypeSafe Jev takes precedence when the key is configured; otherwise this override wins over the strict user-global Pi preferred identity/ordered fallbacks. Planning thinking is capped at medium",
      properties: {
...
async function applyDynamicPlan(pi, ctx, input, project, signal) {
  ...
  }, topologyModels, { typesafeApiKey: process.env.TYPESAFE_API_KEY });
