...
- Added a dependency-free direct TypeSafe Jev preflight adapter: a configured parent-process `TYPESAFE_API_KEY` selects one confirmed fixed-endpoint `jev-latest` typed Choice request before the existing Pi-model fallback policy, deterministically projects bounded role/model/thinking selections into the strict plan, retains body-free versioned provenance and token usage, fails closed without retries or raw provider errors, and strips the dedicated credential before any broker or TUI/RPC worker launch
...
- Added shared Pi and terminal preflight-to-launch admission with separate planning and launch authorization, private body-safe RPC/file transport, deterministic bindings over task/context/project/resolved configuration/planner policy/topology/catalog/resources, mandatory post-preview revalidation, exact preview matching, v8/v9 manifests, and bounded body-free planning provenance across start/list/status/dashboard/Supervisor reads; stale, cancelled, malformed, interrupted, duplicate-session, or unauthorized requests fail before worker launch and dry runs create no retained state
- Added a strict version-1 user-global Pi fallback decision-model policy with canonical preferred identity, ordered exact cross-provider fallbacks, explicit per-candidate thinking capped at `medium`, and configured cancel or separately confirmed static/manual behavior when none are eligible; direct Jev takes precedence when its key is configured, exact per-run Pi overrides follow and precede configured Pi identities when the key is absent, Pi's bounded available/scoped catalog and model-specific thinking support remain authoritative for all workers and Pi fallback planning, and ambiguous or malformed policy fails before provider/orchestration calls
- Added an opt-in model-guided preflight start: `/or-start --plan` and model-tool `dynamicPlan=true` make one separately confirmed bounded decision call before preview to select a fixed-authority worker roster and exact available per-role model/thinking settings, with strict one-shot JSON validation, nested usage accounting for model-tool starts, and no tmux/worker launch before final confirmation
- Added a metadata-only version-1 planner-topology projection that resolves explicit/exact-project/user-global constraints before planner values, preserves exactly one built-in writer and mandatory built-in reviewer, bounds unique optional specialists, admits only exact-project allowlisted custom identities after fresh registry/resource verification, locks custom contracts and read-only authority, validates all selected tuples against Pi's bounded scoped catalog and `medium` cap, and revalidates accepted custom subsets through CLI preview and launch without exposing resource paths or bodies
