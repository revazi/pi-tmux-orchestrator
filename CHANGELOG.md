...
- Bound candidate-set digests and confirmation text to the canonical capability/cost-hint projection so catalog metadata drift after preview rejects launch, missing/zero/malformed rates stay explicit, and retained planning provenance remains body-free; TypeSafe assignment Choice criteria stay identity-only while the same capability objects are sent once as `state.candidate_model_capabilities`
